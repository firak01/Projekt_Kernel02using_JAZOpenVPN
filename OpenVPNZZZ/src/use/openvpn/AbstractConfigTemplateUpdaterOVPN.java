		...
		main:{
			...
			check:{
				...
				if(hmLine==null && objConfigMapper!=null) {
					...
					this.getMainObject().logProtocol( "Creating new configuration file - line(s) by IConfigMapperObject.");
					hmLine = this.getConfigMapperObject().readTaskHashMap();					
				}else {
					this.getMainObject().logProtocol( "Creating new configuration file - line(s) by Hashmap direct.");
				}				
				if(hmLine.isEmpty()){
